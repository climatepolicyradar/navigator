...
class User(Auditable, Base):  # noqa: D101
    ...
    affiliation_organisation = sa.Column(
        sa.String,
    )
    affiliation_type = sa.Column(
        sa.ARRAY(sa.Text),
    )
    policy_type_of_interest = sa.Column(
        sa.ARRAY(sa.Text),
    )
    geographies_of_interest = sa.Column(
        sa.ARRAY(sa.Text),
    )
    data_focus_of_interest = sa.Column(
        sa.ARRAY(sa.Text),
    )
...
class Language(Base):  # noqa: D101
    ...
    part1_code = sa.Column(
        sa.CHAR(length=2),
    )
    part2_code = sa.Column(
        sa.CHAR(length=3),
    )
    name = sa.Column(
...
class Geography(Base):  # noqa: D101
    ...
    value = sa.Column(sa.Text)  # to display to end-users
    official_value = sa.Column(
        sa.Text
...
class Document(Auditable, Base):
    ...
    geography_id = sa.Column(
        sa.SmallInteger, sa.ForeignKey(Geography.geography_id), nullable=False
    )
    action_type_id = sa.Column(
        sa.Integer, sa.ForeignKey(DocumentType.action_type_id), nullable=False
    )
...
class Passage(Base):  # noqa: D101
    ...
    parent_passage_id = sa.Column(
        sa.INTEGER(),
        sa.ForeignKey("passage.passage_id"),
    )
